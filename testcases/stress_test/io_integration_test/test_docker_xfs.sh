#!/bin/bash
# 在docker中反复挂载、卸载xfs文件系统，持续时间由test_time控制，默认12小时
...
mkfs.xfs -f -n ftype=1 /dev/vdb  > /dev/null
rm -rf /tmp/xfs
mkdir -p /tmp/xfs
...
#test the overlayfs+xfs environment(different memory)
...
do
        ...
        if [ $date_now -gt $date_end ]; then
                exit_cmd 0 "docker xfs test done, exit now...."
        fi
done
