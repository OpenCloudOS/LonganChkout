...
Usage: $0 [-c] [-t]
    ...
    -t: Transmitter's IP, default local host

    Warn: Make sure the local host (transmitter) able to ssh the remote host (receivers).
    The receivers should install docker beforehand.
EOF
}
...
ssh root@$transmitter "/root/ping_test.sh.data/ping_test_transmitter.sh"
