...
class Xfstests(Test):
    ...
    def setUp(self):
        ...
        if root_fs in ['ext3', 'ext4']:
            self.use_dd = True
        self.dev_type = self.params.get('type', default='disk') # default='loop'
        ...
        self.devices = []
        self.log_devices = []
        tarball = self.get_data("xfstests-dev-20211207.tar.gz")
        archive.extract(tarball, self.teststmpdir)
        self.sourcedir = os.path.join(self.teststmpdir, "xfstests-dev")
        shutil.copyfile(self.get_data('local.config'),
                        os.path.join(self.sourcedir, 'local.config'))
        shutil.copyfile(self.get_data('group'),
                        os.path.join(self.sourcedir, 'group'))
        if self.fs_to_test == 'ext4':
            shutil.copyfile(self.get_data('exclude_ext4'),
                os.path.join(self.sourcedir, 'extra_exclude.file'))
        elif self.fs_to_test == 'xfs':
            shutil.copyfile(self.get_data('exclude_xfs'),
                os.path.join(self.sourcedir, 'extra_exclude.file'))
        ...
        if self.dev_type == 'loop':
            ...
            self.test_dev = self.params.get('disk_test', default="/dev/vdb")
            self.scratch_dev = self.params.get('disk_scratch', default="/dev/vdc")
            self.devices.extend([self.test_dev, self.scratch_dev])
        # mkfs for devices
        if self.devices:
            cfg_file = os.path.join(self.sourcedir, 'local.config')
...
#        self.teststmpdir = os.path.join(self.teststmpdir, "xfstests-dev")
        ...
        if not self.test_range:
            ...
            if self.exclude or self.gen_exclude or self.share_exclude:
                self.exclude_file = os.path.join(self.sourcedir, 'exclude')
                if self.exclude:
    ...
    def test(self):
        ...
        if not self.test_list:
            ...
            _ex_file = os.path.join(self.sourcedir, 'extra_exclude.file')
            if os.path.isfile(_ex_file):
                args += ' -E %s' % _ex_file
    ...
    def _get_tests_for_group(self, group):
        ...
        group_path = os.path.join(self.sourcedir, 'group')
    ...
    def _get_available_tests(self):
        os.chdir(self.sourcedir)
        tests_set = []
        tests = glob.glob(self.sourcedir + '/tests/*/???.out')
        tests += glob.glob(self.sourcedir + '/tests/*/???.out.linux')
