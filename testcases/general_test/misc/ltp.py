...
class LTP(Test):
    ...
    def setUp(self):
        ...
        self.tmppart = Partition("/dev/vdb", mountpoint="/var/tmp")
        mounted_devs = [line.split()[0]
                   for line in process.getoutput('mount').splitlines()]
        if self.tmppart.device not in mounted_devs:
            self.tmppart.mkfs(fstype="ext4")
            self.tmppart.mount()

        #url = "https://github.com/linux-test-project/ltp/archive/master.zip"
        #tarball = self.fetch_asset("ltp-master.zip", locations=[url])
        tarball = self.get_data("ltp-full-20210927.tar.bz2")
        archive.extract(tarball, self.teststmpdir)
        self.version = os.path.basename(tarball.split('.tar.')[0])
        ltp_dir = os.path.join(self.teststmpdir, self.version)
        os.chdir(ltp_dir)
